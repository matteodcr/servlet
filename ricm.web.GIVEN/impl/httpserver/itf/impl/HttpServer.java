...
public class HttpServer {
    ...
    public static final String RICMLET_URL_BASE = "/ricmlets/";
    private int m_port;
    ...
    public HttpRicmlet getInstanceByURL(String url) throws ReflectiveOperationException, MalformedURLException, URISyntaxException {
        if (url.startsWith(RICMLET_URL_BASE)) {
            var path = url.substring(RICMLET_URL_BASE.length());
            var slashIndex = path.indexOf("/");
            var appName = path.substring(0, slashIndex);
            ...
            return app.getInstance(pkg, appName, ClassLoader.getSystemClassLoader());
        } else {
            return null;
        }
    }
